...
class cuFPMiner_item_hash:
    def __init__(self, file, minSup, sep, file_type, allocator = 'device', shared=False):
        ...
        self.item_start_end_dtype = cp.dtype([
                ('item', cp.int32),
                ('start', cp.int32),
                ('end', cp.int32)
            ])
    ...
    def read_file(self):
        ...
        supports = cp.bincount(csr_data, minlength=max_item + 1)
        
        self.load_factor = 2
        
        self.hash_item_index = cp.zeros(max_item + 1, dtype=self.item_start_end_dtype)
        
        hash_items = cp.zeros(cp.sum(supports) * self.load_factor, dtype=cp.int32)
...
if __name__ == "__main__":
    ...
    obj = cuFPMiner_item_hash("/home/tarun/cuPAMI/datasets/transactional/Transactional_T10I4D100K.csv", 1400, '\t', 'csv', 'managed', True)
    obj.mine()
    obj.printResults()
